...
ORDER BY count(order_details.ProductID) desc;
...
JOIN orders on orders.CustomerID = customers.CustomerID JOIN order_details on order_details.OrderID = orders.OrderID 
...
SELECT CONVERT(AVG(DATEDIFF(orders.ShippedDate,orders.OrderDate)),INT) AS "Délai moyen de livraison en jours" 
FROM orders;
...
CREATE TRIGGER verifp 
    ...
    SIGNAL SQLSTATE '40000' SET MESSAGE_TEXT = "Erreur, au moins 1 produit vien d_un pays étranger";
    END IF;
    END | 
DELIMITER ;
